...
class ApiService {
  ...
  async getDoctorById(doctorId) {
    return this.request(`/api/doctors/${doctorId}`);
  }
  ...
  async getAvailableSlots(doctorId, date) {
    ...
    return this.request(
      `/api/appointments/doctors/${doctorId}/available-slots?selected_date=${dateStr}`,
    );
  }
  ...
  async getAllPatients(params = {}) {
    ...
    return this.adminRequest(
      `/api/admin/patients${queryString ? `?${queryString}` : ""}`,
    );
  }
  ...
  async getAllDoctors(params = {}) {
    ...
    const queryString = queryParams.toString();
    return this.adminRequest(
      `/api/admin/doctors${queryString ? `?${queryString}` : ""}`,
    );
  }

  async updateDoctorApproval(doctorId, approve) {
    return this.adminRequest(
      `/api/admin/doctors/${doctorId}/approval?approve=${approve}`,
      {
        method: "PATCH",
      },
    );
  ...
  async getVideoToken(roomName, participantName) {
    return this.request(
      `/api/video-call/token?room_name=${roomName}&participant_name=${participantName}`,
    );
  }
  ...
  async getDoctorVideoToken(roomName, participantName) {
    const token = localStorage.getItem("doctor_access_token") || "";
    return this.request(
      `/api/video-call/token/doctor?room_name=${roomName}&participant_name=${participantName}`,
      {
        headers: {
          Authorization: token ? `Bearer ${token}` : undefined,
        },
